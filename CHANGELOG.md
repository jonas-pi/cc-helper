...
- **修复 Windows 端预设指令识别问题**
  - 问题：`cc -testapi` 等预设指令被包装函数拼接成字符串后无法识别
  - 原因：包装函数将所有参数拼接为单个字符串，导致 `$firstArg` 解析失败
  - 解决：改进 `$firstArg` 解析逻辑，从拼接的字符串中正确提取第一个参数
  - 添加 `-testapi` 支持（之前只有 `testapi`、`test-api`、`-test`）
  - 效果：所有预设指令（包括带 `-` 前缀的）都能正确识别和执行

- **修复 PowerShell 包装函数参数冲突**
  - 问题：`cc -w` 等命令被 PowerShell 解析为标准参数（`-WarningAction` 等）
  - 原因：包装函数使用了 `param` 声明，导致 PowerShell 参数解析冲突
  - 解决：移除 `param` 声明，直接使用 `$args` 获取所有参数
  - 改进 `-setup` 命令，正确更新现有的包装函数
  - 效果：`cc -w`、`cc -r` 等命令现在可以正常工作
...
- **增强 `cc -del` 命令**
  ...
  - 区分云端 API 模型和本地模型处理：
    - 云端 API 模型：从配置文件中清除
    - 本地模型：删除模型文件并清除配置
  - 自动更新配置文件，确保 `CONFIGURED_MODELS` 和 `MODEL_API_CONFIG` 同步
  - 显示每个模型的类型标记（API/本地）

- **改进 `cc list` 命令**
  - 从配置文件读取模型列表（自动更新）
  - 同时显示云端 API 模型和本地模型
  - 清晰区分模型类型，正确标记当前使用的模型

- **修复 Linux 端 `cc -change` 命令**
  - 正确区分云端 API 模型和本地模型
  - 显示每个模型的类型标记 `[API]` 或 `[本地]`
  - 实现 API 配置的保存和恢复逻辑
  - 切换模型时自动恢复对应的 API 配置
  - 与 Windows 版本功能保持一致
...
- **优化 `cc list` 命令**
  - 同时显示云端 API 模型和本地已下载的模型
  - 清晰区分模型类型，正确标记当前使用的模型
  - 输出更简洁，符合高冷风格

- **增强 `cc -add` 命令**
  - 添加模型后自动更新配置文件（`CONFIGURED_MODELS` 和 `MODEL_API_CONFIG`）
  - 确保新添加的模型能够正确保存和恢复 API 配置
  - 使用原始模型名保存配置，确保正确映射

- **增强 `cc -del` 命令**
  - 删除模型后自动从配置文件中移除
  - 自动清理 `CONFIGURED_MODELS` 和 `MODEL_API_CONFIG`
  - 保持配置文件与实际情况一致

- **重构 `cc testapi` 命令**
  - 逐步测试所有配置的模型（包括云端和本地）
  - 简洁输出：只显示 `模型名 [API类型] ✓/✗/⚠`
  - 最后显示测试统计：`测试完成: X 成功, Y 失败`
  - 无需手动切换模型即可测试所有配置
...
- **同步 Linux 版本功能**
  - 更新 Linux 版本工作模式提示词，添加 NOT_A_COMMAND 判断
  - 添加工作模式非命令输入的友好提示
  - 优化 Token 使用：工作模式 64 tokens，休息模式 256 tokens
  - 确保 Windows 和 Linux 版本功能一致

- **改进帮助信息**
  - 在帮助菜单中添加 `cc -setup` 命令说明（Windows）

### 📝 使用说明

**Windows 用户**：首次使用或遇到参数传递问题时，运行：
```powershell
cc -setup
```
